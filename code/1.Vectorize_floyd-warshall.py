...
def main():
    """Main function to test and validate both Floyd-Warshall implementations."""
    print("Floyd-Warshall Algorithm: Naive vs Vectorized Implementation")
    print("=" * 60)
    ...
    for n in test_sizes:
        ...
        are_equal = np.allclose(result_naive, result_numpy, rtol=1e-10)
        
        print(f"Naive implementation time: {time_naive:.6f} seconds")
        print(f"Numpy implementation time: {time_numpy:.6f} seconds")
        print(f"Speedup: {time_naive/time_numpy:.2f}x")
        print(f"Results are equal: {are_equal}")
        
        if not are_equal:
            print("WARNING: Results differ!")
            print("Max difference:", np.max(np.abs(result_naive - result_numpy)))
        
        print("\nShortest distances matrix (numpy result):")
        print(result_numpy)
        
        if n <= 10:  # Only show detailed comparison for smaller matrices
            print("\nComparison (naive vs numpy):")
            print("Naive result:")
            print(result_naive)
            print("Numpy result:")
            print(result_numpy)
